...
func(a AgentPhase)Update(msg tea.Msg)(AgentPhase,tea.Cmd){
	switch msg:=msg.(type){
	case agentResultMsg:
		if msg.agentStarted&&msg.keyAdded&&msg.errMsg==""{a.step=asStepAskSigning}else{a.step=asStepFailed;a.errMsg=msg.errMsg;if a.errMsg==""{a.errMsg="Required SSH-agent setup could not be verified"};if msg.agentStarted{a.failMenu=NewMenu([]string{"Retry","Continue without signing","Exit"})}else{a.failMenu=NewMenu([]string{"Retry","Exit"})}}
	case tea.KeyMsg:
		switch a.step{
		...
		case asStepFailed:
			switch msg.String(){case "up","k":a.failMenu.Up();case "down","j":a.failMenu.Down();case "enter":a.failMenu.Select();switch a.failMenu.SelectedItem(){case "Retry":a.step=asStepStarting;a.failMenu=MenuModel{};return a,tea.Batch(a.spinner.Tick,startAgentAndAddKey(a.keyPath,a.shellType));case "Continue without signing":a.complete=true;a.step=asStepSuccess}}
		}
