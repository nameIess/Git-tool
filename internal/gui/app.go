...
func (a *App) Startup(ctx context.Context) {
	...
	if err := a.loadSettings(); err != nil {
		logger.Warn("settings load failed: %v", err)
	}
}
...
func (a *App) SetTheme(theme Theme) error {
	if theme != ThemeDark && theme != ThemeWindows {
...
func (a *App) Theme() Theme {
	return a.State().Theme
}

func (a *App) CheckPrerequisites() error {
	for _, name := range []string{"ssh", "ssh-keygen", "ssh-add"} {
		if _, err := runner.Which(platform.OpenSSHBinary(name)); err != nil {
			return err
		}
	}
	if _, err := runner.Which("git"); err != nil {
		return err
	}
	a.setStep("git")
	return nil
...
func (a *App) ConfigureGit(name, email string, global bool) error {
	...
	if name == "" {
		return errors.New("Git name is required")
	}
	if !gitcfg.ValidateEmail(email) {
		return errors.New("a valid Git email is required")
	}
	if err := gitcfg.Apply(name, email, global); err != nil {
		return err
	}
	a.mu.Lock()
	a.state.GitName = name
	a.state.GitEmail = email
	a.state.Step = "ssh"
	a.mu.Unlock()
	return nil
}
...
func (a *App) GenerateSSHKey(passphrase string, overwrite bool) (string, error) {
	...
	if sshkey.Exists(keyPath) && !overwrite {
		keyPath = sshkey.FindNextKeyName(keyPath)
	}
	if err := sshkey.Generate(keyPath, state.GitEmail, passphrase); err != nil {
		return "", err
	}
...
func (a *App) UseExistingSSHKey(path string) error {
	if strings.TrimSpace(path) == "" {
		path = platform.DefaultKeyPath()
	}
	if !sshkey.Exists(path) {
		return fmt.Errorf("SSH key not found: %s", path)
	}
	if err := sshkey.WriteSSHConfig(path); err != nil {
		return err
	}
	a.mu.Lock()
	a.state.KeyPath = path
	a.state.Step = "agent"
	a.mu.Unlock()
	return nil
}
...
func (a *App) SetupSSHAgent() error {
	state := a.State()
	if state.KeyPath == "" {
		return errors.New("SSH key is not selected")
	}
	if err := agent.StartAndAddKey(state.KeyPath, platform.Current()); err != nil {
		return err
	}
	a.mu.Lock()
	a.state.Agent = true
...
func (a *App) GitHubPublicKey() (string, error) {
	...
	key, err := sshkey.ReadPublicKey(state.KeyPath)
	if err != nil {
...
func (a *App) VerifyGitHubSSH() error {
	ssh := platform.OpenSSHBinary("ssh")
	...
	if !strings.Contains(output, "successfully authenticated") {
		return fmt.Errorf("GitHub SSH authentication failed: %s", strings.TrimSpace(result.CombinedOutput()))
	}
	a.mu.Lock()
	a.state.GitHub = true
	a.state.Step = "signing"
	a.mu.Unlock()
	return nil
}
...
func (a *App) ConfigureSigning(enabled bool) error {
	...
	if !enabled {
		a.mu.Lock()
		a.state.Signing = false
		a.state.Step = "complete"
		a.mu.Unlock()
		return nil
	}
	if err := signing.Configure(state.KeyPath, true); err != nil {
		return err
	}
	if _, err := signing.Verify(); err != nil {
		return err
	}
	a.mu.Lock()
	a.state.Signing = true
	a.state.Step = "complete"
	a.mu.Unlock()
	return nil
}

func (a *App) Finish() State {
	a.mu.Lock()
	a.state.Step = "complete"
	a.mu.Unlock()
	return a.State()
...
func (a *App) loadSettings() error {
	...
	if settings.Theme == ThemeDark || settings.Theme == ThemeWindows {
		a.state.Theme = settings.Theme
	}
	return nil
}
