...
func OpenSSHBinary(name string)string{if runtime.GOOS=="windows"{base:=os.Getenv("WINDIR");if base==""{base="C:\\Windows"};p:=filepath.Join(base,"System32","OpenSSH",name+".exe");if _,err:=os.Stat(p);err==nil{return p}};return name}
func EnsureSSHDir()error{return os.MkdirAll(SSHDir(),0700)}
